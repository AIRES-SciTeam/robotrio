...
| `requirements.apt.txt` | Список apt-пакетов, которые устанавливаются в образ `base`. |
| `requirements.py.txt` | Список Python-пакетов, которые устанавливаются в образ `base`. |
| `README.md` | Этот файл. |
...
| Папка | Назначение |
| :--- | :--- |
| `scene` | Файлы gazebo-мира `Scene`. |
| `humanoid` | Файлы для работы гуманоидного робота. |
| `drone` | Файлы для работы дрона. | 
...
| `humanoid` | Контейнер, содержащий базовые пакеты для работы гуманоидного робота OP3.
| `drone` | Контейнер, предназначенный для работы с дроном. Содержит установленные автопилот PX4-Autopilot, DDS-агент для него и необходимые пакеты ros-gz.
...
```sh
docker-compose exec -it <IMAGE_NAME> /bin/bashj
```
